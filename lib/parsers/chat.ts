...
function parseUser(value: unknown): ChatUser | null {
  ...
  const nickname = asString(record.nickname) ?? uniqueId
  const avatarUrl = asString(record.profilePictureUrl)
...
export function normalizeWebcastEvent(eventName: string, data: unknown, at: number = Date.now()): ChatEvent | null {
  ...
  switch (eventName) {
    case "chat": {
      ...
      if (!user) return null
      return { type: "chat", user, comment: asString(record.comment) ?? "", at }
    }
    ...
    case "gift": {
      ...
      const giftName = asString(details?.giftName)
      return {
        type: "gift",
        user,
        giftId,
    ...
    case "error": {
      const info = asString(record?.info)
